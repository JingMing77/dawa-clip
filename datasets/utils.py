...
class DatasetBase:
    ...
    def generate_fewshot_dataset(
        self, *data_sources, num_ways=-1, num_shots=-1, repeat=True
    ):
        ...
        Args:
            ...
            num_ways (int): number of classes to sample. If -1, all classes will be used.
            num_shots (int): number of instances per class to sample, if -1, all instances will be used.
        ...
        for data_source in data_sources:
            ...
            if type(num_ways) == int and  num_ways > 0:
                # Randomly sample a subset of classes
                sampled_labels = random.sample(list(tracker.keys()), num_ways)
                tracker = {label: tracker[label] for label in sampled_labels}
            elif type(num_ways) == list:
                # Use the provided list of labels
                tracker = {label: tracker[label] for label in num_ways if label in tracker}
